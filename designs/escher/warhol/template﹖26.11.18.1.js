...
Design = {
	...
	Resize : function() {
		...
		$(".entry img, .entry iframe, .entry video, .entry object, .entry embed").each(function() {
	...
	Media : function(element) {
		...
		var	entry          = element.closest(".entry"),
			...
			video_parent   = element.closest('.video_component');
		...
		// Fail
			...
			if( element_width > 0 && element_height > 0 ) {
				...
				if( element.is('embed')){
					// do nothing
				} else if (!  element.parents(".slideshow_wrapper").length ) {

					// hack for non-flash videos
	                if ( video_parent.length > 0 && !element.is('object') ){
						video_parent.css({
							"marginLeft": margin_left,
							"height"	: element_height + "px",
							"width"		: element_width + "px"
                        });
						video_parent.find('.mediaplayer').css({
							"height"	: element_height + "px",
							"width"		: element_width + "px"
                        });
    	            } else {
	...
	ResizeSlideshows : function() {
		...
		$(".slideshow_component").each(function() {
			...
			var img    = $(".slideshow_container img:visible", this),
				height = $(img).attr("data-new-height"),
				width  = $(img).attr("data-new-width"),
				margin = $(".slideshow_container img:visible", this).attr("data-margin");
